...
const Search = (props) => {
  ...
  const onFinish = (values) => {
    ...
    const dateArr = ["handleTimeStr"];
    for (let i = 0; i < dateArr.length; i++) {
      const key = dateArr[i];
      if (obj[key])
        obj[key] = [
          moment(obj[key][0]).format("YYYY-MM-DD hh:mm:ss"),
          moment(obj[key][1]).format("YYYY-MM-DD hh:mm:ss"),
        ];
    }
    // 处理输入框
  ...
  return (
    <div className={`${classPrefix}-home-personnel-record-search`}>
      <div className={`${classPrefix}-home-personnel-record-search-content`}>
        <Form {...layout} name="nest-messages" onFinish={onFinish} form={form}>
          <Row>
            ...
            <Col span={colSpan}>
              <Form.Item name={["changesType"]} label="异动类型">
                <Select
                  placeholder="请选择"
                  filterOption={(input, option) =>
                    option.children
                  ...
                  allowClear
                >
                  {props.codeTable.changesType &&
                    props.codeTable.changesType.length &&
                    props.codeTable.changesType.map((it) => (
                      <Option value={it.keyValue} key={it.keyValue}>
                        {it.keyName}
                      </Option>
                    ))}
            ...
            <Col span={colSpan}>
              <Form.Item name={["handleTimeStr"]} label="异动时间">
